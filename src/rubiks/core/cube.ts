...
export class Cube extends Group {
    ...
    public rotateOnePlane(mousePrePos: Vector2, mouseCurPos: Vector2, controlSquare: SquareMesh, camera: Camera, winSize: {w: number; h: number}) {
        ...
        if (!this.state.inRotation) { // 开始触发旋转
            ...
            dbg?.webLineAdd(new Vector2(0, 0), squareDirs[0].screenDir)
            dbg?.webLineAdd(new Vector2(0, 0), squareDirs[1].screenDir)
            dbg?.webLineAdd(new Vector2(0, 0), squareDirs[2].screenDir)
            dbg?.webLineAdd(new Vector2(0, 0), squareDirs[3].screenDir)
            dbg?.webLineAdd(new Vector2(0, 0), screenDir, 'linear-gradient( #ff2222,#ffdddd )')
